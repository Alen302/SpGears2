package SpGears.CompArith.Multiplers
...
class RightShiftMultiplier(k: Int, signed: Boolean = false) extends Component {
...
object SequentialMultiplier { // right-shift version
  ...
  def main(args: Array[String]): Unit = {
    ...
    def sim(signed: Boolean) = {
      SimConfig.withWave.compile(new RightShiftMultiplier(4, signed)).doSimUntilVoid { dut =>
        import dut._

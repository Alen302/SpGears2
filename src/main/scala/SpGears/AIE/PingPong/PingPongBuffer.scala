package SpGears.AIE.PingPong

import SpGears.AIE.Axi4.Axi4Full.{Axi4WriteOnlyMaster, Axi4WriteOnlyMasterSpecRenamer}
import spinal.lib._
...
case class PingPongBuffer(addressWidth: Int = 32, dataWidth: Int = 32, bufferSize: Int = 256) extends Component {
  ...
  val startReceiveClockDomainConfig = ClockDomainConfig(clockEdge = RISING, resetKind = ASYNC, resetActiveLevel = HIGH)
  val startReceiveResetSignal       = Bool(false)
  val startReceiveClockDomain: ClockDomain =
    ClockDomain(clock = inputStartSignal, reset = !ClockDomain.current.readResetWire || startReceiveResetSignal, config = startReceiveClockDomainConfig)
  ...
  val lastBurstCompleteClockDomainConfig = ClockDomainConfig(clockEdge = RISING, resetKind = ASYNC, resetActiveLevel = HIGH)
  val lastBurstCompleteClockDomain: ClockDomain =
    ClockDomain(clock = inputInterruptSignal, reset = !ClockDomain.current.readResetWire || outputStartSignal, config = startReceiveClockDomainConfig)

...
/** this is a stream to axi4 interface define
  *
  * @param addressWidth
  *   The width of address which we use to write data to ddr
  * @param number
  *   The total number of data from fifo in each transfer in a burst
  * @param len
  *   The total number of data transfers within a burst.
  * @param widthPerData
  *   The width of each data from fifo
  */
case class Axi4WriteMaster(addressWidth: Int, len: Int = 256, widthPerData: Int = 32) extends Bundle {
  ...
  when(writeMasterInterface.w.fire) {
    address := Axi4.incr(address, writeMasterInterface.aw.burst, writeMasterInterface.aw.len, writeMasterInterface.aw.size, config.bytePerWord)
  }
  // handshake logic
